...
动机； 性格：既存の枠組に囚われず、問題を見つけて解決する主体性 わくぐみ　とらわれず
具体对策： 收获： 要有数字+序列词
```
...
+ 課題
  - 私のブログサイトはフロントとバック分離するモードで開発したサイトだ。この開発モードを使用にはフロントとバックが別々のサーバにデプロイする必要がある。
    しかし、違うオリジンの間でのリソースリクエストを制限されている。
+ 動機
  - この問題を解決しないと、アドミンサイトからブログサイトを管理することができない。
+ 対策
...
+ ```
  これから「電磁接触器の組立生産」の生産性向上について研究したいと考えている。具体的には、電磁接触器の組立に必要なすべての要素を列挙し、各要素作業の先行作業を設定し、一番効率の良い優先順位をつけなおしてから作業標準書を作成し、そしてシミュレーションソフトを利用してより効果の高い生産方式を研究したいと考えている。
...
```
○情報の価値は、受け手の個性や感性、その時の状況などによって異なる。
本卒研では、文脈や状況に応じて、受け手にとって価値の高い情報を提供す るための方法論やシステム構築のための研究をおこなう。
感性に基づくモノ作り支援
ホームページデザインの印象評価に関する研究
```
...
結果が見えなくても、方法が新しくて面白いと思ったら実行してみる。 

## github

```
https://cno107.github.io/piano3/index.html   ピアノ編曲できるサイト
https://github.com/cno107/personal-Blog  ブログサイト(React Hook + redux + node + mysql+Nginx)
https://cno107.github.io/       githubホームページ(以前作った簡単なサイト)     
https://github.com/cno107/Python--Plane-war　pythonで作った簡単な飛行機大戦ゲーム
```
